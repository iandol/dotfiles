...
void compressFile(const char *inFile, struct stat *inFileInfo, long long int maxSize, int compressionlevel, int minSavings, bool checkFiles)
{
	FILE *in;
	...
	fclose(in);
	outBuf = malloc(filesize + 0x13A + (numBlocks * 8));
	if (outBuf == NULL)
	{
	...
	*(UInt32 *) blockStart = EndianU32_NtoL(numBlocks);
	...
	{
		if (((double) (currBlock - outBuf) / filesize) >= (1.0 - ((float) minSavings / 100)))
		{
			utimes(inFile, times);
			free(inBuf);
			free(outBuf);
...
void decompressFile(const char *inFile, struct stat *inFileInfo)
{
	...
	char *xattrnames, *curr_attr;
	ssize_t xattrnamesize, indecmpfsLen = 0, inRFLen = 0;
	struct timeval times[2];
		...
		for (curr_attr = xattrnames; curr_attr < xattrnames + xattrnamesize; curr_attr += strlen(curr_attr) + 1)
		{
			if (strcmp(curr_attr, "com.apple.ResourceFork") == 0 && strlen(curr_attr) == 22)
			{
				...
				if (inRFLen != 0)
				{
					...
					inRFLen = getxattr(inFile, curr_attr, inBuf, inRFLen, 0, XATTR_SHOWCOMPRESSION | XATTR_NOFOLLOW);
					if (inRFLen < 0)
					{
...
void printUsage()
{
	printf("afsctool 1.1.5 (build 19)\n"
...
int main (int argc, const char * argv[])
{
	...
	int printVerbose = 0, compressionlevel = 5, minSavings = 25;
	...
	ssize_t xattrnamesize, xattrsize;
	mode_t outFileMode;
	void *attr_buf;
	UInt16 big16;
	UInt64 big64;
	...
	if (applycomp && (argc - i > 1))
	{
		sscanf(argv[i], "%d", &minSavings);
		if (minSavings > 99 || minSavings < 1)
		{
			fprintf(stderr, "Invalid minimum savings percentage; must be a number from 1 to 99\n");
			return -1;
		}
		i++;
	}
			...
			if (xattrnamesize > 0)
			{
				...
				for (curr_attr = xattrnames; curr_attr < xattrnames + xattrnamesize; curr_attr += strlen(curr_attr) + 1)
				{
						...
						xattrsize != 0)
					{
						...
						xattrsize = getxattr(fullpath, curr_attr, attr_buf, xattrsize, 0, XATTR_SHOWCOMPRESSION | XATTR_NOFOLLOW);
						if (xattrsize < 0)
						{
							fprintf(stderr, "getxattr: %s\n", strerror(errno));
							free(xattrnames);

#!/usr/bin/env bash
# One-way mirror: macOS folderA -> Ubuntu destination
# Run first with: DRY_RUN=1 ./sync-folderA-mac-to-ubuntu.sh

set -Eeuo pipefail

# ---------- EDIT THESE ----------
REMOTE_HOST="${REMOTE_HOST:-cog5z}" # use ssh config for defining the login values
SSH_PORT="${SSH_PORT:-2222}"
SRC="${SRC:-$HOME/Dropbox/References/}"          # trailing slash = copy contents of folderA
DEST="${DEST:-/home/cog5/References/}"              # trailing slash recommended
REMOTE_BACKUP_ROOT="${REMOTE_BACKUP_ROOT:-/home/cog5/.rsync-backups/folderA}"
# -------------------------------

SYNC_NAME="Refs-mac-to-ubuntu"
LOG_DIR="${LOG_DIR:-$HOME/Library/Logs/folder-sync}"
...
PATH="$HOME/.pixi/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:$PATH"
